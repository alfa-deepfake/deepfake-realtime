...
Здесь два независимых варианта:

| | Где | Для чего |
|---|---|---|
| **портируемый** | корень репозитория | поднять с нуля на любой машине с NVIDIA GPU |
| **серверный** | `server-mounted/` | то, что **сейчас работает** на A100 `62.183.4.208` |
...
Собрано и поднято с нуля на A100 (Ubuntu 24.04, driver 595, Docker 29.6.1):
оба контейнера дошли до `healthy`, отдают HTTP 200, AlphaFace поднялся на CUDA
со всеми пятью ONNX на `CUDAExecutionProvider`, seed-vc — `[vc] model loaded,
sr=22050`. Тестовое окружение после проверки удалено.
